	...
	virtual tsl::elm::Element* createUI() override {
		...
		list->addItem(new tsl::elm::CustomDrawer([](tsl::gfx::Renderer *renderer, s32 x, s32 y, s32 w, s32 h) {
			if (!SaltySD) {
				...
				if (!(ReverseNX_RT->pluginActive)) renderer->drawString("GameNotCheckAnyModeGuiTestCustomDrawerText"_tr.c_str(), false, x, y+40, 18, renderer->a(0xF33F));
				else {
	...
	virtual void update() override {
		...
		if (PluginRunning) {
			if (i > 9) {
				...
				if (!ReverseNX_RT->res.handheld_res) strcpy(HandheldDDR, "UpdateDefaultHandleDDRGuiTestListItemText"_tr.c_str());
				else snprintf(HandheldDDR, sizeof(HandheldDDR), "UpdateHandleDDRGuiTestListItemText"_tr.c_str(), resolutions[ReverseNX_RT->res.handheld_res].first, resolutions[ReverseNX_RT->res.handheld_res].second);
				if (!ReverseNX_RT->res.docked_res) strcpy(DockedDDR, "UpdateDefaultDockedDDRGuiTestListItemText"_tr.c_str());
				else snprintf(DockedDDR, sizeof(DockedDDR), "UpdateDockedDDRGuiTestListItemText"_tr.c_str(), resolutions[ReverseNX_RT->res.docked_res].first, resolutions[ReverseNX_RT->res.docked_res].second);
			}
			else i++;
		}
	}
